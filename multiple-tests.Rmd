...
So power to detect this effect would be:
...
If we assume alpha = .05, two-tailed (which we are), then **power is about .85** to detect the population correlation of .30 with any 100 random observations we drew from that population. This is roughly consistent what we found based on the empirical demonstration - in the above example, the correlation test was significant in 169 of the 200 simulated random samples from the population (that is, 84.5% of the random samples resulted in a significant correlation, so **our empirically observed power here was .845**).
